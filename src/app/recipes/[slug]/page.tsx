...
import { useState, useEffect } from 'react'; // useEffect might be needed if data fetching were more complex client-side
import { getRecipeData, getAllRecipeSlugs } from '@/lib/recipes';
import type { Recipe, ValueUnit, Fermentable, Hop, Yeast, Misc } from '@/types/recipe';
...
export async function generateStaticParams() {
  const slugs = getAllRecipeSlugs(); // This uses fs, so it's server-side at build time.
...
const RecipeStepsDisplay: React.FC<{ markdownContent?: string }> = ({ markdownContent }) => {
  if (!markdownContent) {
    return (
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <p className="text-muted-foreground">No detailed steps provided for this recipe.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
     <Card>
        ...
        <CardContent>
          <article className="prose prose-sm sm:prose-base lg:prose-lg xl:prose-xl 2xl:prose-2xl max-w-none dark:prose-invert text-foreground">
        ...
        <CardContent className="p-6">
          <Tabs defaultValue="details" className="w-full">
            <TabsList className="grid w-full grid-cols-1 sm:grid-cols-2 mb-4">
              <TabsTrigger value="details" className="flex items-center gap-2 data-[state=active]:shadow-md">
                <FileText size={16} /> Recipe Details
              </TabsTrigger>
              <TabsTrigger value="steps" className="flex items-center gap-2 data-[state=active]:shadow-md">
            ...
            <TabsContent value="steps">
              <RecipeStepsDisplay markdownContent={recipe.stepsMarkdown} />
            </TabsContent>
          </Tabs>
        </CardContent>
      </Card>
